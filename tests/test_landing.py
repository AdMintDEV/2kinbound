...
def test_html_does_not_call_provider_apis() -> None:
    blob = INDEX + AUDIT
...
def test_audit_stub_mentions_ac0b_gate() -> None:
    lower = AUDIT.lower()
    assert "ac#0b" in lower
    assert "invent" in lower


def test_server_serves_home_post_audit_and_health() -> None:
    ...
    try:
        ...
        get_stub = urlopen(f"{base}/audit", timeout=3).read().decode("utf-8")
        req = Request(
            f"{base}/audit",
            data=b"url=https://www.example.com&brand=Northwind",
            method="POST",
        )
        posted = urlopen(req, timeout=3).read().decode("utf-8")
        health = urlopen(f"{base}/healthz", timeout=3).read().decode("utf-8")
        assert "Inbound Score" in home
        assert "$29" in home
        assert "not live" in get_stub.lower() or "not calling" in get_stub.lower()
        assert "stub" in posted.lower()
        assert "https://www.example.com" in posted
        assert "none (not invented)" in posted.lower() or "not invented" in posted.lower()
